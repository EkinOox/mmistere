...
const Boxprod = () => {
    ...
    useEffect(() => {
        document.body.classList.add('body-background-boxprod');
        const audio = new Audio(backgroundAudio);
        audio.loop = true;
        audio.play();
        return () => {
            document.body.classList.remove('body-background-boxprod');
            document.body.classList.remove('body-background-boxprod-tableau');
            audio.pause();
            audio.currentTime = 0;
        };
    }, []);
    ...
    useEffect(() => {
        const showTextTimeout = setTimeout(() => {
            setDisplayText(<><FeatherIcon icon='map-pin' size="30" strokeWidth="2" className="icon"
                                          style={{fill: "transparent", stroke: "red"}}/> Box de production</>);
        }, 1000);
        ...
        const showNewImgTimeout = setTimeout(() => {
            document.body.classList.add('body-background-boxprod-azzura-tips');
            setButtonText("Avancer");
    ...
    const handleButtonClick = () => {
        document.body.classList.remove('body-background-boxprod-azzura-tips');
        document.body.classList.add('body-background-boxprod-tableau');
    ...
    const handleCreaItemClick = () => {
        setShowPopup(true);
        setPopupContent({
            image: "../src/assets/img/crea/projet-crea2.png",
            title: "Evolution",
            text: "Lors de notre deuxième année, nous avons dû réaliser un grand projet en groupe. Le but était de créer des agences sur le thème de la cigale." +
                "L’agence de développement web Ask Ip nous a permis de développer le sens du travail d'équipe et d'être pluridisciplinaire." +
    ...
    const handleDevItemClick = () => {
        setShowPopup(true);
        setPopupContent({
            image: "../src/assets/img/dev/projet-dev2.png",
            title: "Construction",
            text: "Lors de notre deuxième année, nous avons dû réaliser un grand projet en groupe. Le but était de créer des agences sur le thème de la cigale." +
                "L’agence de développement web Ask Ip nous a permis de développer le sens du travail d'équipe et d'être pluridisciplinaire." +
    ...
    const handleComItemClick = () => {
        setShowPopup(true);
        setPopupContent({
            image: "../src/assets/img/com/projet-com2.png",
            ...
            {showChoice && (
                <>
                    ...
                    <div className="object-container1">
                        <div className="objet1" onClick={handleComItemClick}>Accomplissement</div>
                        <div className="objet2" onClick={handleCreaItemClick}>Evolution</div>
                        <div className="objet3" onClick={handleDevItemClick}>Construction</div>
                    </div>
                    {
                        showPopup && (
                            <Popup
